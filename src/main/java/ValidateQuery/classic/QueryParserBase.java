...
//      throw e;
  ...
  /**
   * By default QueryParser uses {@link MultiTermQuery#CONSTANT_SCORE_REWRITE}
  ...
  protected void addMultiTermClauses(List<BooleanClause> clauses, Query q) {
    ...
    if (allNestedTermQueries) {
      ...
      Occur occur = operator == OR_OPERATOR ? Occur.SHOULD : Occur.MUST;
      if (q instanceof BooleanQuery) {
        for (BooleanClause clause : ((BooleanQuery)q).clauses()) {
  ...
  protected Query newFieldQuery(Analyzer analyzer, String field, String queryText, boolean quoted)  throws ParseException {
    Occur occur = operator == Operator.AND ? Occur.MUST : Occur.SHOULD;
